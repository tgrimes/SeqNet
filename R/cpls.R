...
#' @export
run_cpls <- function(x, threshold = NULL, v = 1, parallel = TRUE) {
  if(parallel) {
...
#' @export
run_cpls_parallel <- function(x, threshold = NULL, v = 1, parallel = FALSE) {
  n <- nrow(x) #Number of observations.
  p <- ncol(x) #Number of genes.
  if(v >= n) {
    stop("v must be less than n (the number of rows in X)")
  ...
  s_list <- foreach::`%dopar%`(foreach::foreach(i = 1:p), {
